@if (count($raffles['data']))
    ...
    <tr class="{{ $tr_background }}" data-id="{{ $raffle['raffle_id'] }}" >
        ...
        <td>
            <button type="button" id="toggle-edit-raffle" class="btn btn-default" data-id="{{ $raffle['raffle_id'] }}" data-name="{{ $raffle['raffle_name'] }}" data-description="{{ $raffle['description'] }}" data-start="{{ $raffle['start_date'] }}" data-end="{{ $raffle['end_date'] }}" data-toggle="tooltip" data-placement="bottom" title="Edit"><span class="glyphicon glyphicon-edit"></span></button>
